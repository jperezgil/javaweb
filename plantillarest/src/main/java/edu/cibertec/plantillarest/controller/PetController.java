...
@RestController
@RequestMapping("/pets")
@Tag(name = "Pets", description = "Api Pets")
public class PetController {
    ...
    @GetMapping("/export-pdf/{nombre}")
    public ResponseEntity<byte[]> exportPdfPorNombre(@PathVariable String nombre) throws JRException, FileNotFoundException {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_PDF);
        headers.setContentDispositionFormData("petsReport", nombre+".pdf");
        return ResponseEntity.ok().headers(headers).body(petService.exportPdfPorNombre(nombre));
    }
